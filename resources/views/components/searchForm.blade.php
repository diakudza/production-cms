<form action="{{route('search.part')}}" class="card w-full bg-base-100 shadow-xl">
    <div class="grid grid-cols-6 gap-4 p-1">
        <input type="text" placeholder="Номер детали" name="partNumber" class="input w-full max-w-xs"
               value="{{ $searchPartNumber ?? NULL}}"/>

        <select class="select w-25 max-w-xs" name="machine_id">
            <option disabled selected>По станкам</option>
        ...
        <select class="select w-25 max-w-xs" name="author">
        ...
        <select class="select w-25 max-w-xs" name="partType">
        ...
        <select class="select w-15 max-w-xs" name="itemOnPage">
